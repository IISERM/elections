...
<body ng-controller="settings" style="background-image: url(<?php echo URL::base()."/img/pattern.png"; ?>), url(<?php echo URL::base()."/img/100_9048.jpg"; ?>);">
	<center>
		<input type="text" ng-model="config.student.search" placeholder="Filter" />
		<span>		
		...
		<table>
			...
			<tr ng-repeat="student in students | filter:config.student.search | orderBy:config.student.orderBy:config.student.reverse">
				...
				<td>
					<select>
						<option selected="selected" value="{{indexify.batch[student.batch]}}">{{student.batch}}</option>
						<option value="{{indexify.batch['07']}}">07</option>
						<option value="{{indexify.batch['08']}}">08</option>
						<option value="{{indexify.batch['09']}}">09</option>
						<option value="{{indexify.batch['10']}}">10</option>
						<option value="{{indexify.batch['11']}}">11</option>					
					</select>
				</td>
				<td>
